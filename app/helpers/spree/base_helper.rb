module Spree
  module BaseHelper
    ...
    def link_to_cart(text = nil)
      ...
      text = text ? h(text) : ('basket')
      css_class = nil

      if current_order.nil? or current_order.line_items.empty?
        text = "#{text} <span>(#{t('0')})</span>"
        css_class = 'empty'
      else
        text = "#{text} <span>(#{current_order.item_count})</span>  <span class='amount' style='display: none'>#{order_subtotal(current_order)}</span>".html_safe
        css_class = 'full'
      end
    ...
    def get_sign_shapes_for_product(product)
      SignBaseShape.all
    end
